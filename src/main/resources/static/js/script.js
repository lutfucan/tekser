$(document).ready(function() {
    checkPageSizes();
    changePageAndSize();
    changePageAndSizeClients();
    searchUsersOnEnterKeyPressed();
    searchClientsOnEnterKeyPressed();
    searchReceiptsOnEnterKeyPressed();
    changeActiveLinks();
    keepSearchParametersAfterPageRefresh();
    keepSearchParametersAfterPageRefreshClients();
    keepSearchParametersAfterPageRefreshReceipts()
});
...
function changeActiveLinks() {
    ...
    if(menuItem.includes("clients")){
        $("a[href='/adminPage/clients']").parent("li").addClass("active");
    }
    if(menuItem.includes("receipts")){
        $("a[href='/adminPage/receipts']").parent("li").addClass("active");
    }
}
...
function changePageAndSize() {
    $('#pageSizeSelect').change(function(evt) {
        ...
        if (value != null && value !== "") {
            window.location.replace("/adminPage/users?usersProperty=" + selectedProperty +
                "&propertyValue=" + value + "&pageSize=" + this.value + "&page=1");
        }
...
function changePageAndSizeClients() {
    $('#pageSizeSelect').change(function(evt) {
        let selectedProperty = $("#search-client-dropdown option:selected").text();
        let value = $("#searchClientBar").val();

        if (value != null && value !== "") {
            window.location.replace("/adminPage/clients?clientsProperty=" + selectedProperty +
                "&propertyValue=" + value + "&pageSize=" + this.value + "&page=1");
        }

        else{
            window.location.replace("/adminPage/clients?pageSize=" + this.value + "&page=1");
        }
    });
}

function changePageAndSizeReceipts() {
    $('#pageSizeSelect').change(function(evt) {
        let selectedProperty = $("#search-receipt-dropdown option:selected").text();
        let value = $("#searchReceiptBar").val();

        if (value != null && value !== "") {
            window.location.replace("/adminPage/receipts?receiptsProperty=" + selectedProperty +
                "&propertyValue=" + value + "&pageSize=" + this.value + "&page=1");
        }

        else{
            window.location.replace("/adminPage/receipts?pageSize=" + this.value + "&page=1");
        }
    });
}
...
function searchClientsOnEnterKeyPressed(){
    $("#searchClientBar").keypress(function (event) {
        if (event.which === 13) {
            searchClientByProperty();
        }
    });
}

function searchReceiptsOnEnterKeyPressed(){
    $("#searchReceiptBar").keypress(function (event) {
        if (event.which === 13) {
            searchReceiptByProperty();
        }
    });
}
...
function keepSearchParametersAfterPageRefreshClients(){
    $("#searchClientBar").val(getSavedValueForTextBox("searchClientBar"));
    $("#search-client-dropdown").val(getSavedValueForDropDown("search-client-dropdown"));

    function getSavedValueForTextBox  (v){
        let clientsPropertyParam = new URL(location.href).searchParams.get('clientsProperty');
        if (localStorage.getItem(v) === null) {
            return "";
        }
        else if(clientsPropertyParam === null){
            return "";
        }
...
function keepSearchParametersAfterPageRefreshReceipts(){
    $("#searchReceiptBar").val(getSavedValueForTextBox("searchReceiptBar"));
    $("#search-receipt-dropdown").val(getSavedValueForDropDown("search-receipt-dropdown"));

    function getSavedValueForTextBox  (v){
        let receiptsPropertyParam = new URL(location.href).searchParams.get('receiptsProperty');
        if (localStorage.getItem(v) === null) {
            return "";
        }
        else if(receiptsPropertyParam === null){
            return "";
        }
